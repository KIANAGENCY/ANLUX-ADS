...
/**
 * Este módulo es client-safe a propósito (lo usa `hooks/use-alerts.ts` para
 * el camino DEMO): no debe importar nada de `lib/meta/real/` (server-only).
 * El equivalente para cuentas REALES vive en `real-engine.ts`, que reutiliza
 * las reglas de negocio de aquí pero corre exclusivamente en el servidor.
 */
...
/** Reglas de negocio sobre campañas (comunes a demo y real): CTR, CPC, gasto sin resultados. */
...
export function applyCostPerResultOutlierRule(
  alerts: PerformanceAlert[],
...
export function sortBySeverity(alerts: PerformanceAlert[]): PerformanceAlert[] {
...
/**
 * Evalúa las reglas de negocio sobre las campañas de un cliente DEMO
 * (dataset simulado — ver `lib/mock/`) y genera alertas de performance.
 */
export async function generateMockAlerts(clientId: string, range: DateRange): Promise<PerformanceAlert[]> {
